...
import { getReasoningLoops } from '../../lib/api';
...
import { Brain, Plus, History, Activity, Database } from 'lucide-react';
...
export default function ReasoningPage() {
    ...
    useEffect(() => {
        ...
        fetchHistory();
    }, [dataMode, activeDebateId]);

    useEffect(() => {
        if (!activeDebateId || dataMode === 'empty') return;
    ...
    }, [activeDebateId, subscribe, dataMode]);

    const activeDebate = debates.find(d => d.id === activeDebateId);
    const isLive = activeDebate?.status === 'running';

    return (
        <div className="h-screen flex flex-col bg-[#0a0e27] text-white overflow-hidden p-6 space-y-6 pt-16 lg:pt-6">
            <DemoModeBanner />
            ...
            {dataMode === 'empty' ? (
                <EmptyState
                    ...
                    title="Reasoning Engine Idle"
                    message="Intelligence synthesis requires primary LLM configuration. Please setup an API Provider."
                />
            ) : (
                <>
                    {/* HEADER */}
                    <header className="flex justify-between items-center bg-[rgba(26,31,58,0.8)] backdrop-blur-[20px] p-6 rounded-xl border border-[rgba(0,255,136,0.2)] shadow-[0_0_20px_rgba(0,255,136,0.1)] shrink-0">
                        ...
                        <NeonButton onClick={() => setIsModalOpen(true)} className="px-4 py-2 sm:px-6 sm:py-3 whitespace-nowrap">
                            <Plus className="w-4 h-4 mr-2" />
                            <span className="hidden sm:inline">Start New Reasoning</span>
                    ...
                    <div className="flex-1 flex flex-col lg:flex-row gap-6 overflow-hidden">
                        ...
                        <div className="w-full lg:w-80 flex flex-col bg-[rgba(26,31,58,0.8)] rounded-xl border border-[rgba(0,255,136,0.2)] backdrop-blur-[20px] overflow-hidden lg:h-full h-1/3 min-h-[200px] shrink-0">
                        ...
                        <div className="flex-1 flex flex-col gap-6 overflow-hidden">
                            ...
                            <div className="h-48 bg-[rgba(26,31,58,0.8)] rounded-xl border border-[rgba(0,255,136,0.2)] backdrop-blur-[20px] p-4 flex flex-col shrink-0 relative overflow-hidden hidden md:flex">
